...
GPIO__liquid_level = 26 # liquid sensor, input
...
GPIO_cooler = 23 # GPIO23: PWM output, cooler
GPIO_heater = 24 # GPIO24: PWM output, heater
GPIO_air_pump = 25 # GPIO25: PWM output, air pump
GPIO_fan = 18 # GPIO18: output, fan
GPIO_liquid_pump = 8 # GPIO8: output, liqiud pump
...
P_1 = 1, I_1 = 0.1, D_1 = 0.05 # PID_1
P_2 = 10, I_2 = 0.1, D_2 = 0.05
P_3 = 10, I_3 = 0.1, D_3 = 0.05
flow_set = 200 # SCCM
flow_sensor_zero = 1 # volts
flow_sensor_fullscale = 5 # volts
...
GPIO.setup(GPIO_OPC, GPIO.IN) # GPIO8 as an input, counter
GPIO.setup(GPIO__liquid_level, GPIO.IN) # GPI18 as an input, liquid level
GPIO.setup(GPIO_cooler,GPIO.OUT) # GPIO23 as an output, cooler
GPIO.setup(GPIO_heater,GPIO.OUT) # heater
GPIO.setup(GPIO_air_pump,GPIO.OUT) # air pump
GPIO.setup(GPIO_fan,GPIO.OUT) # fan
GPIO.setup(GPIO_liquid_pump,GPIO.OUT) # liquid pump
...
def get_liquid_level(level_stat_arr): # record liquid level
	np.roll(level_stat_arr, -1)
	level_stat_arr[0] = GPIO.input(GPIO__liquid_level)
...
def liquid_pump_act(level_start_arr, pump_stat_arr, switch_cycle): # swith pump if matchs condition
	...
	np.roll(pump_stat_arr, -1)
	if level_start_arr.sum() == 0: # if liquid empty last enough long time
		if stat == stat[0]: # if few cycle all the same then switch
			liquid_pump(pump_stat_arr[0])
...
while True:
	...
	flow_rate = 1000*(flow_voltage-flow_sensor_zero)/(flow_sensor_fullscale-flow_sensor_zero) # SCCM
	print('flow = %d' % flow)
	...
	if dc_2 > 100: # duty cycle should between 0-100
		...
		dc_2 = 0
	dc_3 = pid_2(flow_rate) # air pump duty cycle
